...
// ✅ ENHANCED MAPPING: Используем новый маппинг с учетом Color, Size, Brand
...
export function mapShopifyOrderToBitrixDeal(order) {
  ...
  if (order.line_items && Array.isArray(order.line_items)) {
    ...
    for (const item of order.line_items) {
      ...
      // ===== ENHANCED MAPPING LOGIC (используется новый маппинг с учетом Color, Size, Brand) =====
      // Try enhanced SKU mapping first (with Color, Size, Brand matching)
      const productIdFromEnhanced = item.sku ? skuMappingEnhanced[item.sku] : null;
      ...
      // Use enhanced mapping with fallback to old mapping
      let productId = productIdFromEnhanced || productIdFromOldMapping || productIdFromHandle || productIdFromConfig || null;
      
      // Log if enhanced mapping was used
      if (productIdFromEnhanced && !productIdFromOldMapping) {
        console.log(`[ORDER MAPPER] ✅ Enhanced mapping used for SKU: ${item.sku} -> Product ID: ${productIdFromEnhanced}`);
      }
